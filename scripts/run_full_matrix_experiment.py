"""Run the full 16-scenario Phase 1 experiment matrix and save artifacts.

This script runs:
- 1 deterministic baseline
- 15 stochastic scenarios with 1000 Monte Carlo trajectories each

Outputs are written under data/experiment_results/:
...
from core.scenarios import (
    ALL_SCENARIOS_BAT,
    DIST_ONLY_SCENARIOS_BAT,
    LAMBDA_LEVELS,
    NOISE_ONLY_SCENARIOS_BAT,
    P_DIST_LEVELS,
)
...
def _build_summary_rows(
    ...
    for name, result in ordered_results:
        config = result.scenario_config if isinstance(result, BatchResult) else result.scenario_config
        if config is None:
            ...
            row.update(
                {
                    ...
                    "vol_total_mean": (
                        result.terminal_yield.vol_total if result.terminal_yield else 0.0
                    ),
                }
            )
...
def _generate_figures(
    ...
    disturbance_only_results = {
        ...
        for config in DIST_ONLY_SCENARIOS_BAT
        if config.name in stochastic_results
    }
    noise_only_results = {
        config.name: stochastic_results[config.name]
        for config in NOISE_ONLY_SCENARIOS_BAT
...
def main() -> None:
    ...
    manifest = {
        ...
        "seed": SEED,
        "n_trajectories": N_TRAJECTORIES,
        "scenario_names": [config.name for config in ALL_SCENARIOS_BAT],
        "scenarios": [asdict(config) for config in ALL_SCENARIOS_BAT],
    }
