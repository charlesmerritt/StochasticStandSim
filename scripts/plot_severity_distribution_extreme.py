"""Plot the Beta(3, 3) extreme severity distribution with annotated thresholds.

Produces a publication-quality figure matching the style of the moderate
Beta(3.6, 8.4) severity distribution plot.
"""
...
def main() -> None:
    ...
    # Main density curve
    ax.plot(x, pdf, color="#2c3e50", linewidth=2.0, label="Beta(3, 3) PDF")
    ...
    ax.set_title(
        r"Disturbance Severity Distribution: Beta($\alpha$=3, $\beta$=3)",
        fontsize=13,
        fontweight="bold",
    )
    ax.set_xlim(0, 1.0)
    ...
    ax.text(0.82, max(pdf) * 0.92, textstr, fontsize=9, verticalalignment="top",
    ...
    fig.tight_layout()
    out_path = OUT_DIR / "severity_distribution_extreme.png"
