"""Run salvage sensitivity analysis with median severity threshold.

Same design as run_salvage_sensitivity.py but with the salvage trigger
set at the median of Beta(3.6, 8.4) ≈ 0.2886 instead of p75.

Outputs are written under data/salvage_sensitivity_median/.
"""
...
N_TRAJECTORIES = 1000
SEED = 42
OUTPUT_DIR = Path("data/salvage_sensitivity_median")

SEVERITY_MEAN = 0.30
SEVERITY_KAPPA = 12.0
ALPHA = SEVERITY_MEAN * SEVERITY_KAPPA   # 3.6
BETA = (1 - SEVERITY_MEAN) * SEVERITY_KAPPA  # 8.4
SALVAGE_THRESHOLD = float(beta_dist.ppf(0.50, ALPHA, BETA))  # median
...
def _build_scenarios() -> list[ScenarioConfig]:
    """Build 6 scenarios: 3 disturbance-only × {no salvage, salvage at median}."""
...
def _plot_validation(
    ...
    for i, label in enumerate(base_labels):
        ...
        colors_list.extend(["#4C72B0", "#55A868"])
    ...
    ax.set_title(
        f"NPV: No Salvage vs Salvage (threshold = {SALVAGE_THRESHOLD:.3f}, median)"
    )
    ...
    legend_elements = [
        ...
        Patch(facecolor="#55A868", alpha=0.7, label="Salvage at median"),
    ...
    fig.tight_layout()
    fig.savefig(fig_dir / "salvage_npv_comparison_median.png", dpi=150)
    plt.close(fig)
    print(f"Saved validation figure: {fig_dir / 'salvage_npv_comparison_median.png'}")


def main() -> None:
    _ensure_dir(OUTPUT_DIR)

    print(f"Salvage severity threshold (median): {SALVAGE_THRESHOLD:.4f}")
    ...
    manifest = {
        ...
        "n_trajectories": N_TRAJECTORIES,
        "salvage_severity_threshold": SALVAGE_THRESHOLD,
        "salvage_threshold_quantile": "p50 (median)",
        "salvage_price_fraction": 0.50,
        "beta_alpha": ALPHA,
        "beta_beta": BETA,
    ...
    print("\n" + "=" * 70)
    print("SALVAGE SENSITIVITY RESULTS (MEDIAN THRESHOLD)")
    print("=" * 70)
