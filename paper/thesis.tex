...
In this work we address three key questions that challenge the use of deterministic modeling of pine stand development.
...
In this work we address two important questions related to ackowledging uncertainty in pine stand development.
...
Stumpage and action prices are also stationary and defined by the latest literature \textcite{costsMaggardNatzke}. We impose the following feasibility constraints: $\text{TPA} \geq 100$, $\text{BA} \geq 0$, $\text{HD}$ is non-decreasing, and $\text{Age} > 0$, where TPA is trees per acre, BA is basal area per acre, and HD is dominant tree height.
...
Stand-level optimization in forestry begain in the 1970's using dynamic programming and simulation methods. The determination of the optimal rotation age of even-aged stands of trees as well as optimal thinning regimes were of initial interest. Later, optimizing stand density become an important consideration \parencite{bettingerDensitydependentStandlevelOptimization2005}.
\begin{itemize} 
...
Markov Decision Processes (MDPs) provide a robust mathematical framework for modeling forest management under conditions of inherent ecological and economic risk. Unlike deterministic models that assume a known future, MDPs treat forest evolution as a series of stochastic transitions between discrete states. These states are typically defined by biophysical variables such as basal area, tree size, and species composition. 
\begin{itemize}
...
Focused on the Pacific Northwest, this study integrates carbon offset prices directly into an MDP objective function. It finds that while moderate carbon prices enhance species and size diversity, but high prices may inadvertently promote monocultures as the forest is managed primarily for carbon.
...
This article introduces Graph-based Markov Decision Processes to address the spatial complexities of wind damage risk in Swedish forests. By accounting for how neighboring stands provide topographical shelter, the model optimizes management policies to increase the expected NPV of stands.
...
A pine stand is harvested at a predetermined rotation age $T$. No intermediate treatments are applied unless explicitly specified. We evaluate a rotation age of 35 years to represent a plausible range for loblolly pine plantations owned by private forest landowners in the Southern United States.
...
\manualnumberedsection{Disturbance Module}
...
The occurence of a disturbance in a given year is an independent Bernoulli trial with probability $p_{dist}$, where $p_{dist}$ is defined as the mean return interval in years. This corresponds to a description of disturbances as "occuring once every \textit{n} years" where \textit{n} is the return interval.
